...
describe('Cms', () => {
  ...
  it('renders an empty list of strings', () => {
    ...
    expect(
      wrapper.element.querySelector('#string-resource-list').children.length
    ).toBe(0)
  })
  ...
  it('renders a non empty list of strings', () => {
    const mocks = {
      err: false,
      data: {
        strings: [{ name: 'n1', value: 'v1' }, { name: 'n2', value: 'v2' }]
      }
    }
    ...
    Array.from(
      wrapper.element.querySelector('#string-resource-list').children
    ).forEach((item, index) => {
  ...
  it('renders error message', () => {
    ...
    expect(wrapper.element.querySelector('#error-message').innerHTML).toContain(
      mocks.err
    )
  })
